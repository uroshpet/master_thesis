...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if cfg.STOP_SIGN_DETECTOR:
        ...
        V.add(StopSignDetector(cfg.STOP_SIGN_MIN_SCORE,
              ...
              outputs=['pilot/throttle', 'cam/image_array'], threaded=True)
        V.add(ThrottleFilter(), 
              inputs=['pilot/throttle'],
              outputs=['pilot/throttle'])
    
    if cfg.PURE_PURSUIT_CONTROLLER:
        from donkeycar.parts.pure_pursuit.object_detector_pp import ObjectDetector_PP
        from donkeycar.parts.pure_pursuit.trajectory_creator import TrajectoryCreator
        from donkeycar.parts.pure_pursuit.pure_pursuit import PurePursuit
        V.add(ObjectDetector_PP(camera_dimentions = (cfg.IMAGE_W,cfg.IMAGE_H)), inputs = ['cam/image_array'], outputs = ['detected_object','detected_image'], threaded = True)
        V.add(TrajectoryCreator((cfg.IMAGE_W , cfg.IMAGE_H), cfg.PURE_PURSUIT_QUALITY), inputs = ['detected_object','cam/image_array'], outputs = ['trajectory','trajectory_image'], threaded = True)
        V.add(PurePursuit(cfg.PURE_PURSUIT_LOOKAHEAD_DISTANCE, (cfg.IMAGE_W,cfg.IMAGE_H)), inputs = ['trajectory'], outputs = ['pure_steering'], threaded = True)
    #
    ...
    class DriveMode:
        def run(self, mode,
                    ...
                    pilot_angle, pilot_throttle, pure_steering):
            if mode == 'user':
                ...
                return pure_steering, user_throttle

    V.add(DriveMode(),
          inputs=['user/mode', 'user/angle', 'user/throttle',
                  'pilot/angle', 'pilot/throttle','pure_steering'],
...
def add_user_controller(V, cfg, use_joystick, input_image='trajectory_image'):
    """
